...
kernel void set_rows_f16_i64_f16(global char * src0,
                                 ulong         offset0,
                                 global char * src1,
                                 ulong         offset1,
                                 global char * dst,
    ...
    const int i11 = (ne11 == ne01) ? i01 : (i01 % ne11);
    const int i12 = (ne12 == ne02) ? i02 : (i02 % ne12);
    const int i13 = (ne13 == ne03) ? i03 : (i03 % ne13);
    ...
    const int nvec = nblk0 >> 3;
    global const uint4 * s4 = (global const uint4*) src0_ptr;
    global uint4*       d4 = (global uint4*)       dst_ptr;
    for (int i = i00; i < nvec; i += lsz0) {
        d4[i] = s4[i];
    }
    for (int i = (nvec << 3) + i00; i < nblk0; i += lsz0) {
