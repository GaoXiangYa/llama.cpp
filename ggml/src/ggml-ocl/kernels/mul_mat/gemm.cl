...
kernel void gemm(
    ...
    for (int k = 0; k < ne00; k += BK) {
        ...
        if (global_a_row < ne01 && global_a_col < ne00) {
            lA[local_col * BK + local_row] = *(global float*)(src0 + global_a_col * nb00 + global_a_row * nb01 + i01 * nb02 + i02 * nb03);
        } else {
        ...
        if (global_b_row < ne11 && global_b_col < ne10) {
            lB[local_row * BN + local_col] = *(global float*)(src1 + global_b_col * nb10 + global_b_row * nb11 + i11 * nb12 + i12 * nb13);
        } else {
        ...
        for (int kk = 0; kk < BK; ++ kk) {
            sum += lA[local_col * BK + kk] * lB[local_row * BK + kk];
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
