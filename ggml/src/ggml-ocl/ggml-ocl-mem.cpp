...
ggml_ocl_tensor_extra * ocl_extra_pool::alloc() {
    ...
    ggml_ocl_tensor_extra * extra = new ggml_ocl_tensor_extra();
    all_.push_back(extra);
    return extra;
}

void ocl_extra_pool::reset() {
    for (ggml_ocl_tensor_extra * extra : all_) {
        extra->reset();
        free_.push_back(extra);
    }
}
